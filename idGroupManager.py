...
class idGroupManager(object):
    ...
    def checkArnold(*args):
        ...
        if( mc.getAttr( 'defaultRenderGlobals.currentRenderer' ) != 'arnold' ):
            mc.confirmDialog( t="Warning", message= "Please set current renderer to Arnold!", icon='critical' )
            sys.exit( "Please set current renderer to Arnold!" )
    ...
    def buildUI(self,*args):
        ...
        mc.rowLayout(nc = 3, cw3 = (300,300,300), adjustableColumn = 3, cal = [(1,'center'),(2,'center'),(3,'center')],columnAttach=[(1, 'both', 0), (2, 'both', 0),(3,'both',0)])
        mc.button(l='Optimize ID Attributes', c = self.optimizeIdAttr)
        mc.button(l='Delete ID Attributes', c = self.deleteIdAttr)
        mc.button(l='Create AOVs', c = self.createCustomAovs)
        return widget
    
    def getAssetName(self):
        rcnull = mc.ls(type="rigCenterNode")
        if len(rcnull) == 0:
            assetName = ''
            return assetName
        if len(rcnull) == 1:
            assetName = mc.getAttr(rcnull[0]+".assetString") + '_'
            return assetName
        else:
            mc.confirmDialog( t="Warning", message= "There are more than one rcnull node in the scene. Please check before proceed.", icon='critical' )
            sys.exit( "More than one revull node in the scene!" )
    ...
    def optimizeIdAttr(self,*args):
        setList = self.listIdSets()
        setList.sort()
        #clean up color attributes
        for node in setList:
            memberList = self.listIdSetsMember([node])
            memberMeshList = self.getMeshList(memberList)
            self.deleteColorAttr(memberMeshList)
        #add color attributes
        for node in setList:
            idColorNum = node.split('_')[-2:][0]
            memberList = self.listIdSetsMember([node])
            memberMeshList = self.getMeshList(memberList)
            self.addColorAttr(memberMeshList, idColorNum)
    ...
    def createCustomAovs(self,*args):
        ...
        for node in setList:
            ...
            idColorNum = node.split('_')[-2:][0]
            colorDataNodeName = self.assetName + 'colorData_IdGroup_' + idColorNum
            aovName = self.assetName + 'ID_group_' + idColorNum
            if not mc.objExists(colorDataNodeName):
                colorNode = mc.shadingNode('aiUserDataColor', asShader=True, name = colorDataNodeName)
                mc.setAttr(colorNode+'.colorAttrName', self.assetName + 'idColor_' + idColorNum, type = 'string')
            if not mc.objExists('aiAOV_' + aovName):
                aovs.AOVInterface().addAOV( aovName )
                mc.connectAttr(colorDataNodeName+'.outColor','aiAOV_'+aovName+'.defaultValue')
